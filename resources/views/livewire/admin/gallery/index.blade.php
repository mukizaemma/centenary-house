...
<div>
    <div class="bg-light rounded p-4">
        <div class="d-flex flex-wrap align-items-center justify-content-between mb-3">
            <h4 class="mb-2">Media Gallery</h4>
            <div class="d-flex flex-wrap gap-2">
                ...
                >
                <select class="form-select form-select-sm" wire:model="typeFilter" style="min-width: 120px;">
                        ...
                        <tr>
                            <td>
                                @if($item->type === 'image' && $item->image_path)
                                    <img src="{{ asset($item->image_path) }}" alt="" class="rounded border" style="height: 50px; width: auto; max-width: 100px; object-fit: cover;">
                            ...
                            <td>{{ $item->title ?: '—' }}</td>
                            <td class="text-center"><span class="badge bg-secondary">{{ $item->type }}</span></td>
                            ...
                            <td class="text-end">
                                <button type="button" class="btn btn-sm btn-outline-primary me-1" wire:click="edit({{ $item->id }})">
                                    <i class="fa fa-edit"></i>
                                </button>
                                <button type="button" class="btn btn-sm btn-outline-danger" wire:click="delete({{ $item->id }})" onclick="return confirm('Delete this item?')">
            ...
            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ $editingId ? 'Edit Gallery Item' : 'New Gallery Item' }}</h5>
                        <button type="button" class="btn-close" wire:click="closeFormModal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form wire:submit.prevent="save" id="gallery-form">
                            <div class="mb-3">
                            ...
                            @if($type === 'image')
                                <div class="mb-3">
                                    ...
                                    @endif
                                    @if(!$editingId)
                                        <small class="text-muted d-block">Required for image type.</small>
                                    @endif
                                </div>
                            @else
                                <div class="mb-3">
                                    <label class="form-label">Video URL</label>
                                    <input type="url" class="form-control" wire:model.defer="video_url" placeholder="https://youtube.com/...">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary" form="gallery-form">
